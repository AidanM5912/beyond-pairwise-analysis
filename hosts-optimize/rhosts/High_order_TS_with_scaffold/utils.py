...
def parse_input(input):
    ...
    for s in range(n_input):
        ...
        if sys.argv[s] == '--no-zscore':
            # ->  do not z-score the original data
            do_zscore = False
        if input[s] == '--output-s3':
            output_s3_path = input[s + 1]


    t_total = [t for t in range(t_init, t_end)]

    return(path_file, t_init, t_end, t_total, ncores, null_model_flag,
           flag_edgeweight, flag_edgeweight_fn, javaplex_path, scaffold_path, do_zscore, output_s3_path)
...
def load_data(path_single_file):
    ...
    if extension_file == 'mat':
        ...
        raise ValueError(f"Unsupported file type: .{extension_file}")
    return data
    # print(np.shape(data))
...
class simplicial_complex_mvts():
    def __init__(self, multivariate_time_series, null_model_flag, folder_javaplex, scaffold_outdir, do_zscore=True):
        ...
        # Computing the z-score of the initial data and replace the variable self.raw_data
            ...
            print("No z-score applied to the data")
    ...
    def compute_edges_triplets(self):
        ...
        # DEBUG: report problem size
        print(f"[DEBUG] compute_edges_triplets: num_ROI={self.num_ROI}, T={self.T}, N_edges={N_edges}")

        # Allocate edge stats
        self.ets_zscore = np.zeros((N_edges, 2))
        self.ets_max = np.zeros((self.T,))

        # DEBUG: report allocation info
        print(f"[DEBUG] Allocated ets_zscore shape={self.ets_zscore.shape}, ets_max shape={self.ets_max.shape}")

        l_index_prev = 0
        l_index_next = self.num_ROI - 1

        for i in range(self.num_ROI):
            # DEBUG: looping indices
            print(f"[DEBUG] Edge batch {i}: indices [{l_index_prev}:{l_index_next}]")

            c_prod = self.raw_data[u[l_index_prev:l_index_next]] * self.raw_data[v[l_index_prev:l_index_next]]
            # compute mean & std across time for each edge
            self.ets_zscore[l_index_prev:l_index_next] = np.vstack((
                np.mean(c_prod, axis=1),
                np.std(c_prod, axis=1)
            )).T

            # DEBUG: show first few zscore stats
            if i == 0:
                print(f"[DEBUG] ets_zscore[0:3] first rows =\n{self.ets_zscore[:3]}")

            # update the max‐over‐time series
            self.ets_max = np.max(
                np.vstack((self.ets_max, np.abs((c_prod - self.ets_zscore[l_index_prev:l_index_next, 0][:, None]) /
                        self.ets_zscore[l_index_prev:l_index_next, 1][:, None]))
                ), axis=0)

            l_index_prev = l_index_next
            l_index_next += (self.num_ROI - i - 2)

        # DEBUG: finished edges
        #print(f"[DEBUG] Finished edges: ets_zscore dtype={self.ets_zscore.dtype}, ets_max dtype={self.ets_max.dtype}")

        # Save edge index mapping
        ...
        # Allocate triplet stats
        self.triplets_ts_zscore = np.zeros((N_triplets, 2))
        self.triplets_max = np.zeros((self.T,))

        # DEBUG: report triplet allocation
        #print(f"[DEBUG] Allocated triplets_ts_zscore shape={self.triplets_ts_zscore.shape}, triplets_max shape={self.triplets_max.shape}")

        l_index_prev = 0
        l_index_next = int((self.num_ROI - 1) * (self.num_ROI - 2) / 2)

        for i in range(self.num_ROI):
            # DEBUG: looping triplet batch
            #print(f"[DEBUG] Triplet batch {i}: indices [{l_index_prev}:{l_index_next}]")

            c_prod = (
                self.raw_data[u[l_index_prev:l_index_next]] *
                self.raw_data[v[l_index_prev:l_index_next]] *
                self.raw_data[w[l_index_prev:l_index_next]]
            )
            self.triplets_ts_zscore[l_index_prev:l_index_next] = np.vstack((
                np.mean(c_prod, axis=1),
                np.std(c_prod, axis=1)
            )).T
            ...
            self.triplets_max = np.max(
                np.vstack((self.triplets_max, np.abs((c_prod - self.triplets_ts_zscore[l_index_prev:l_index_next, 0][:, None]) /
                        self.triplets_ts_zscore[l_index_prev:l_index_next, 1][:, None]))),axis=0)

            l_index_prev = l_index_next
            l_index_next += int((self.num_ROI - i - 2) * (self.num_ROI - i - 3) / 2)

        # DEBUG: finished triplets
        #print(f"[DEBUG] Finished triplets: triplets_ts_zscore dtype={self.triplets_ts_zscore.dtype}, triplets_max dtype={self.triplets_max.dtype}")
